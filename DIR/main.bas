...
dim shared as fb.IMAGE ptr    lockedpanpic, modulepic, pickerpic, pickerbackpic, labsceneImg, lockeddoorpic, openeddoorpic
...
constructor BUG
   with THIS
      ...
      ._bugImgW      => 100
      ._bugImgH      => 70
      ._bugImage     => imageCreate (._bugImgW, ._bugImgH, rgba(12,12,12, 180), 32)
   end with
   THIS.InitBug()
   scope
      ...
      dim as double	btn2Delay				    => 2.
      '
      THIS._bugButton => _
            TT3.DBUTTON( topLeftCornerPositionX2, topLeftCornerPositionY2, _
                     btn2Width, btn2Height, _
                     buttonText2, _ 
                     btn2Behav, _
                     btn2Delay   )
   end scope
...
sub BUG.InitBug()
   for i as integer = 1 to 4 + rnd()*4
      ...
      line THIS._bugImage, _ 
            (x, y)-step(16, 16), _ 
            rgba(180,c,78, 177), _ 
            bf
      line THIS._bugImage, _ 
            (THIS._bugImgW - x - 20, y)-step(16, 16), _ 
            rgba(180,c,78, 177), _ 
            bf
   next i
   draw string THIS._bugImage, (8, 8), THIS._bugName, rgba(100,100,30, 80)
   '
   dim as integer    keywordAmount  = rnd()*38 + 1
...
function BUG.TestForNodeContent(byval N as integer) as string
   ...
   if N>-1 andAlso N<=uBound(THIS._arrayOfKeywords) then
...
dim as single     xx              'left/right
dim as single     yy              'up/down
dim as single     dx              'x-vector
dim as single     dy              'y-vector
dim as single     xi              'x-direction
dim as single     yi              'y-direction
dim as single     x0              'x-origin
dim as single     y0              'y-origin
dim as single     nTilesX         'nTilesX
dim as single     nTilesY         'nTilesY
dim as single     tileTrueWidth   'tileTrueWidth, in case of isometric tiles
dim as single     tileTrueHeight  'tileTrueHeight, in case of isometric tiles
tileTrueWidth     => tileW
...
#macro _LABPICKER()
   draw string (400, 108), "active key", rgba(100,200,100,190)
   draw string (400, 124), "" & left(pickedKeyword, 12), rgba(100,200,100,190)
   if adnShowUp andAlso activeAdnNodeIndex>-1 then
      ...
      scope
            ...
            scope
               ...
               for xi as long = -1 to (nTilesX) + 0
                for yi as long = -1 to (nTilesY) + 0
                  line labsceneImg, (x0 + xi*tileW, y0 + yi*tileH)-step(tileW,tileH), _
                       rgb(100,100,200), _
                       b
         ...
         draw string labsceneImg, (6, 6), str(xx) & ";" & str(yy), rgba(195,205,0, 100)
         
         line  labsceneImg, _ 
         (200 + xx - geneZoneXmin, 160 + yy - geneZoneYmin)-_ 
         (200 + xx - geneZoneXmax, 160 + yy - geneZoneYmax), _ 
         rgba(100,0,0,80), _ 
         bf
         if xx>geneZoneXmin      andAlso _
            xx<geneZoneXmax      andAlso _ 
            yy>geneZoneYmin      andAlso _ 
            yy<geneZoneYmax      then
               line  labsceneImg, _ 
               (200 + xx - geneZoneXmin, 160 + yy - geneZoneYmin)-_ 
               (200 + xx - geneZoneXmax, 160 + yy - geneZoneYmax), _ 
               rgba(0,220,100,40), _ 
         ...
         'catch user keyboard interaction
         'dx = 0
         'dy = 0
         if multikey(fb.SC_SPACE) and TIMER>(startDelay + 0.2) then
           showCoordinate = not showCoordinate
           startDelay = TIMER
         end if
         if multikey(&H1E) or multikey(&H4B) then dx += +.1 'left
         if multikey(&H20) or multikey(&H4D) then dx += -.1 'right
         if multikey(&H11) or multikey(&H48) then dy += +.1 'up
         if multikey(&H1F) or multikey(&H50) then dy += -.1 'down
         while inkey<>"": wend
...
#macro _SHOWPICKUPDIALOG()
   if isShowPickupDialogUp then
      line ((dskW - 390)/2, 180)-step(400,140), rgba(50,200,195, 120), bf
      line ((dskW - 390)/2, 180)-step(400,200), rgba(150,125,25, 20), b
      line ((dskW - 390)/2, 280)-step(400,0), rgba(150,125,25, 120)
      line ((dskW - 390)/2, 284)-step(400,0), rgba(150,125,25, 120)
...
#macro  _SHOWSCREENUNLOCKPICKEDUPDIALOG()
   line ((dskW - 590)/2, 220)-step(600,140), rgba(160,150,155, 255), bf
   line ((dskW - 590)/2, 220)-step(600,140), rgba(160,125,125, 200), b
   line ((dskW - 590)/2 + 2, 220 + 2)-step(600 - 4,140 - 4), rgba(60,105,25, 160), b
   line ((dskW - 590)/2, 300)-step(600,0), rgba(150,125,25, 120)
   line ((dskW - 590)/2, 314)-step(600,0), rgba(150,125,25, 120)
   '
   draw string ((dskW - 390)/2 - 8, 230), "FXM: Tremendous!!!", rgb(0,0,0)
   draw string ((dskW - 390)/2 - 8, 258), "     You did it.", rgb(0,0,0)
   draw string ((dskW - 390)/2 - 8, 274), "     Bring the SCREENUNLOCK to me at the doors now", rgb(0,0,0)
   draw string ((dskW - 390)/2 - 8, 314), "     Hurry up!", rgb(0,0,0)
   scope
...
#macro _LABPAGE()
      ...
      screenSet 0, 1
         ...
         if searchButton._mouseClick then
            ...
            '   ? i, bugTest.TestForNodeContent(i)
            'next i
            'flip
         ...
         if pickedKeyword="SCREENUNLOCK" then
            _SHOWSCREENUNLOCKPICKEDUPDIALOG()
         end if
         '_________________________________________________________________________
...
#macro _LABINIT()
   ...
   ''dev.note: contains dsktop size - what if changed while prog running?
   scope
      ...
      buttonText2 = "LEAVE"
      showPickupButtonCancel => _
            TT3.DBUTTON( topLeftCornerPositionX2 + 150, topLeftCornerPositionY2, _
                     btn2Width, btn2Height, _
                     buttonText2, _ 
                     btn2Behav, _
                     btn2Delay   )
...
#macro _SHOWFIXDOORDIALOG()
   if isShowPickupDialogUp then
      ...
      line ((dskW - 590)/2, 314)-step(600,0), rgba(150,125,25, 120)
      '
      if not screenUnlockPickedUp then
         draw string ((dskW - 390)/2 - 8, 230), "FXM: This door is locked!!!", rgb(0,0,0)
         draw string ((dskW - 390)/2 - 8, 258), "     Go to the laboratory, and extract a SCREENLOCK from a bug", rgb(0,0,0)
         draw string ((dskW - 390)/2 - 8, 274), "     Then bring it back, I'll be able to free our crew members!", rgb(0,0,0)
         draw string ((dskW - 390)/2 - 8, 314), "     Hurry up, they don't have much oxygen left!", rgb(0,0,0)
      else
         draw string ((dskW - 390)/2 - 8, 230), "FXM: Et voila!", rgb(0,0,0)
      end if
      scope
         put ((dskW - 790)/2, 200), fxm, TRANS
      end scope
...
#macro _DOORSCENE()
   if facingFrontDoorScene andAlso not screenUnlockPickedUp then
   ...
   '
   screenSet 0, 1
      line (0,40)-(1919,1079), rgba(190,160,135, 12), bf
      
      line ((dskW - 390)/2, 280)-step(400,400), rgba(150,150,105, 12), bf
      for i as integer = 0 to 5
         line ((dskW - 390)/2, 280 + i * 24)-step(400,400 - i*24), rgba(150,155,150 - i*4, 42), bf
      next i
      ...
      scope
         ...
         if not screenUnlockPickedUp then
            ...
            imageInfo openeddoorpic, imgW, imgH
            put ((dskW - imgW)/2, (dskH - imgH)\2 + 40), openeddoorpic, TRANS
         end if
      end scope
...
'...images loading
   ...
   openeddoorpic  => CreditFBDoc.BMPlOAD(exePath() & "\pic\dooropen.bmp")
   ...
   labsceneImg    => imageCreate (400, 320, rgba(5,0,55, 55), 32)
   dim as BUG  bugTest
